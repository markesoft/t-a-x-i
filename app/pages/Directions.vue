<template>
  <div id="Directions">
    ...
    <div v-for="address in addressData" :key="address">
      <div @click="storeAddress(address)" class="flex items-center custom-border-bottom">
        <div class="bg-gray-400 mx-5 my-3.5 p-1.5 rounded-full">
          <MapMarkerIcon :size="30" fillColor="#f5f5f5" />
        </div>
        <div>
          <div class="text-lg text-gray-600">{{ address.properties.name }}</div>
          <div class="text-lg text-gray-400">{{ address.properties.label }}</div>
        </div>
      </div>
    </div>
...
const storeAddress = (address) => {
  if (isPickupActive.value === true) {
    direction.pickup = address.geometry.coordinates[0] + "," + address.geometry.coordinates[1]
    pickup.value = address.properties.name
    addressData.value = []
  } else {
    direction.destination = address.geometry.coordinates[0] + "," + address.geometry.coordinates[1]
    destination.value = address.properties.name
  }
...
const findAddress = debounce(async (address) => {
  try {
    ...
    const res = await $fetch('/api/adress/' + address)
    addressData.value = res.features || []
  } catch (err) {
    console.log(err)
  }
}, 300)
